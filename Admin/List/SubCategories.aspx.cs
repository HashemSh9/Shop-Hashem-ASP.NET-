    ...
    public partial class SubCategories : System.Web.UI.Page
    {
        ...
        ["const"].ToString());
           ...
           if(Qureysql!="")
            ...
            {//في حالة لم يقم بعملية بحث يتم عرض بشكل عادي
                string sql = "SELECT s.Subcate_Id, s.Subcate_Name, s.Subcate_Photo, c.Cate_Name FROM TbSubcategory s INNER JOIN TbCategories c ON s.Cate_Id = c.Cate_Id";
        ...
        protected void Lnk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect("~/Admin/Edit/Edit_SubCategories.aspx?id=" + id);
            }
            catch
            {
        ...
        bool DeleteRecord(string id)
        {
            try
            {
                // يتم حدف الفئة الفرعية وجميع البيانات المرتبطة بها في قاعدة البيانات.
        ...
        void GetData()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    SqlDataAdapter da = new SqlDataAdapter("SELECT s.Subcate_Id, s.Subcate_Name, s.Subcate_Photo, c.Cate_Name FROM TbSubcategory s INNER JOIN TbCategories c ON s.Cate_Id = c.Cate_Id", con);
        ...
        protected void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else// في حالة قام بكتابة شيء
                {
                    ...
                    string query = "SELECT s.Subcate_Id, s.Subcate_Name, s.Subcate_Photo, c.Cate_Name FROM TbSubcategory s INNER JOIN TbCategories c ON s.Cate_Id = c.Cate_Id WHERE s.Subcate_Id LIKE '%" + @searchTerm + "%' OR s.Subcate_Name LIKE '%" + @searchTerm + "%' OR c.Cate_Name LIKE '%" + @searchTerm + "%'";
