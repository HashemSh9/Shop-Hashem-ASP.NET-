<%@ Page Title="الفئات الفرعية" Language="C#" MasterPageFile="~/Admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="SubCategories.aspx.cs" Inherits="Shop_College.Admin.List.SubCategories" %>
            ...
            <div class="table-responsive">
                <div class="table-wrapper">
                    ...
                    <div class="col-6 col-xl-4">
                        <h2><b>إدارة</b> الفئات الفرعية</h2>
                    </div>
...
<%--                OnTextChanged="txtSearch_TextChanged"--%>
    ...
    " placeholder="ادخل اسم او رقم" aria-label="Recipient's username" aria-describedby="basic-addon2" ></asp:TextBox></div> 
                        <a href="../Add/Add_SubCategories.aspx" class="btn btn-secondary"><i class="material-icons">&#xE147;</i> <span style="font-size:10px">إضافة فئة فرعية جديدة</span></a>
            ...
            <ContentTemplate>
                    ...
                    <asp:Repeater ID="Repeater1" runat="server"  >
                        ...
                        <ItemTemplate>
                                <tr>
                                   <td>
                                              <asp:Label ID="Label1" runat="server" Text='<%# Eval("Subcate_Id") %>'></asp:Label>
                                       
                                          </td>
                                         <td>
                                            <asp:Label ID="statusSpan" runat="server" Text='<%# Eval("Subcate_Name") %>'></asp:Label>
                                        </td>
                                          <td>
                                             

                            <%--يتم تخزين الصورة، كما موجودة في القاعدة، لكي نتمكن من الوصول اليها--%>
                        <asp:Image ID="Image2" runat="server" ImageUrl='<%# "../../" + Eval("Subcate_Photo") %>' class="avatar" alt="Avatar" />
                                        </td>
                                             <td>
                                                 <asp:Label ID="lblCatename" runat="server" Text=' <%# Eval("Cate_Name")%>'></asp:Label>
                                        </td>
                                        <td>
                                            <asp:LinkButton ID="Lnk" runat="server"  CommandArgument='<%# Eval("Subcate_Id") %>' OnClick="Lnk_Click" class="settings" title="تعديل" data-toggle="tooltip" ><i class="material-icons">&#xE8B8;</i></asp:LinkButton>


                                            <asp:LinkButton ID="BtnDel"  runat="server" CommandArgument='<%# Eval("Subcate_Id") %>' 
                                                OnClientClick = "return confirm('هل أنت متأكد من حدف الفئة الفرعية؟! سوف يتم حدف جميع البيانات المتصلة بها!.');"
