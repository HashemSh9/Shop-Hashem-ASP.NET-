...
namespace Shop_College.Admin.Edit
{
    ...
    public partial class Edit_SubCategories : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {   //الحصول ع رقم الاي دي ، في اول مرة 
                ...
                else
                {
                    ...
                    Response.Redirect("~/Admin/List/SubCategories.aspx");
        ...
        void GetData()
        {
            try
            {
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    string sql = "select * from TbSubcategory where Subcate_Id = @ca";
                    ...
                    while (dr.Read())
                    {
                        ...
                        txtName.Text = dr["Subcate_Name"].ToString();
                        Message_Box.Text = dr["Subcate_Description"].ToString();
                        blah.ImageUrl = "~/" + dr["Subcate_Photo"].ToString();
                        drp1.SelectedValue = dr["Cate_Id"].ToString();
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    while (dr.Read())
                    {
                        //يتم جلب البيانات ووضعهن في المكان المناسب
                        // يتم إرجاع
                        //نقوم بعملية المقارنة للتأكد من انه الإسم تم تغييره او لا 
                        if (name.Trim() == dr["Subcate_Name"].ToString().Trim())
                            return true;
                    ...
                    //في حال كان الإسم مختلف وتم تغييره
                    return false;
                }
            }
            catch
        ...
        protected int CheckNameinDB(string Name)
        {
            ...
            try
            {       //في حال تغير محتوي النص، يتم البحث ع الإسم للتأكد من انه غير موجود من قبل
                using (SqlConnection con = new SqlConnection(cs))
                {
        ...
        string SaveImg(FileUpload imageName, string name)
        {
            ...
            try
            {
                ...
                {//إعطاء للصورة اسم فريد لكي لا يتحدث تضارب في الأسماء لنفس الصورة
                    ...
                    //الحصول علي مسار الصورة بعد حفظها
                    return imgpath;
                ...
                //في حال كان هناك  خطا في حفظ البيانات
                return "ops";
            }
        ...
        {
            //-------------//
            //لكي يتم حل مشكلة، في حالة انه الصورة قد تم حفظها من دون بياناتها
            ...
            try
            {
                ...
                if (oldNameimg != "ops")
                {
                    using (SqlConnection con = new SqlConnection(cs))
                    {
                        ...
                        String query = "UPDATE TbSubcategory SET Subcate_Name =@Sn,Subcate_Photo=@Sp, Subcate_Description=@S_Dec  , Cate_Id=@Cid where Subcate_Id=@SID";
                        SqlCommand cmd = new SqlCommand(query, con);


                        //إرسال الصورة لحفظها في السيرفر وإرجاع اسمها الجديد التي تم حفظه
                        ...
                        if (newName != "ops")
                        {
                            ...
                            cmd.Parameters.AddWithValue("@Cid", drp1.SelectedValue);

                            cmd.Parameters.AddWithValue("@Sn", C_Name);
                            cmd.Parameters.AddWithValue("@SID", idQuery);
                            cmd.Parameters.AddWithValue("@S_Dec", C_Dec);
        ...
        private int SaveDataoneNoimg(String C_Name, String C_Dec)
        {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    ...
                    String query = "UPDATE TbSubcategory SET Subcate_Name =@Sn, Subcate_Description=@S_Dec , Cate_Id=@Cid where Subcate_Id=@SID";
                    SqlCommand cmd = new SqlCommand(query, con);
                            ...
                            if (IsValidLength(Message_Box.Text, 255))
                            {
                                ...
                                if (!fileImg.HasFile)
                                {
                                    ...
                                    if (SaveDataoneNoimg(txtName.Text.Trim(), Message_Box.Text.Trim()) == 1) { 
                                        ScriptManager.RegisterStartupScript(this, this.GetType(), "aa", "UpdateDoneSubCate()", true);
