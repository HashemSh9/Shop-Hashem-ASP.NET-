...
namespace Shop_College
{
    ...
    public partial class All_SubCategories : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["IdSubcate"] != null && Request.QueryString["IdSubcate"] != string.Empty)
                {
                    //تخزين قيمة الاي دي
                    string idsubcat = Request.QueryString["IdSubcate"].ToString();



                    Session["SetDropStuct"] = " where c.Cate_Id = "+idsubcat;

                }
                else
                {
                    Session["SetDropStuct"] = " where s.Subcate_Id != -1";
                }
        ...
        ["const"].ToString());
            ...
            string sql = "SELECT s.Subcate_Id, s.Subcate_Name, s.Subcate_Description, s.Subcate_Photo,c.Cate_Id ,c.Cate_Name, (SELECT COUNT(*) FROM TbAds a WHERE a.Subcate_Id = s.Subcate_Id and a.Ads_Status=1) AS AdsCount FROM TbSubcategory s JOIN TbCategories c ON s.Cate_Id = c.Cate_Id " + HttpContext.Current.Session["SetDropStuct"].ToString() ;
        ...
        protected void imgpoto_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                Response.Redirect("~/All_Ads.aspx?Idforads=" + id);
            }
            catch (Exception)
            {
        ...
        void SetFilter()
        {
            ...
            if (IsEmptyOrspace(valSerch))
                Session["SetDropStuct"] = "  where  s.Subcate_Name LIKE '%" + valSerch + "%' or  c.Cate_Name LIKE '%" + valSerch + "%' ";
            else
                Session["SetDropStuct"] = "  where   s.Subcate_Id != -1 ";
